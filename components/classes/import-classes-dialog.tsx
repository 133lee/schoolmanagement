...
const REQUIRED_HEADERS = ["name", "gradeName"];
const VALID_HEADERS = ["name", "gradeName", "capacity"];
...
function downloadTemplate() {
  const csv = "name,gradeName,capacity\nA,Grade 8,40\nB,Grade 8,40\nA,Form 1,35\n";
...
export function ImportClassesDialog({ open, onOpenChange, onSuccess }: ImportClassesDialogProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (data.data.successful > 0) {
        toast({ title: "Import complete", description: `${data.data.successful} classes imported` });
        onSuccess?.();
      }
    } finally {
      setImporting(false);
      ...
      <DialogContent className="sm:max-w-[520px]">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <span className="font-mono text-xs">gradeName</span> (must match exactly, e.g.{" "}
            <span className="font-mono text-xs">Grade 8</span>). Optional:{" "}
            <span className="font-mono text-xs">capacity</span> (default 40).
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          {!result && (
            <div
              ...
              onDrop={onDrop}
              onClick={() => document.getElementById("class-csv-input")?.click()}>
              <input id="class-csv-input" type="file" accept=".csv" className="hidden" onChange={onFileChange} />
            ...
            <div className="space-y-3">
              <div className={cn("flex items-center gap-2 p-3 rounded-lg border", result.failed === 0 ? "bg-green-50 border-green-200" : "bg-amber-50 border-amber-200")}>
                ...
                <div className="text-sm space-y-0.5">
                  {result.successful > 0 && <p className="font-medium text-green-700">{result.successful} class{result.successful !== 1 ? "es" : ""} imported</p>}
          ...
          <div className="flex justify-end gap-3 pt-1">
            ...
            {!result && (
              <Button onClick={handleImport} disabled={!parsedRows.length || importing}>
                ...
                Import {parsedRows.length > 0 ? `${parsedRows.length} Classes` : ""}
              </Button>
            )}
